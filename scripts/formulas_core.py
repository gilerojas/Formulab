...
def validar_formula_escalada(
    ...
    gal_objetivo: float,
    pg_esperado: float,
    tolerancia_cant: float = 0.5,
    tolerancia_gal: float = 0.1,
    ...
    CHECKS CRÍTICOS:
    1. Σ(CANT) debe ser ≈ 100%
    2. Σ(GL_PRO) debe ser ≈ gal_objetivo
    3. P/G calculado debe ser ≈ P/G esperado
    
    Args:
        df: DataFrame con fórmula escalada
        gal_objetivo: Galones solicitados
        pg_esperado: P/G de la fórmula maestra
        tolerancia_cant: % de error aceptable en suma CANT (default: 0.5%)
        tolerancia_gal: Galones de error aceptable (default: 0.1)
        tolerancia_pg: Error aceptable en P/G (default: 0.5)
    
    Returns:
        Tupla (is_valid, issues, metrics)
        - is_valid: True si todas las validaciones pasan
        - issues: Lista de warnings/errores detectados
    ...
    # ==================== CHECK 4: Ingredientes Válidos ====================
    # Verificar que no haya valores negativos o NaN
